//SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;

import {Test, console} from "forge-std/Test.sol";
import {MoodNft} from "../src/MoodNft.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";

contract TestMoodNft is Test {
    string public constant HAPPY_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string public constant SAD_IMAGE_URI =
        "data:image/svg+xml;base64,";

    address USER = makeAddr("user");
    address BOB = makeAddr("bob");

    MoodNft moodNft;

    modifier UserMintMoodNft() {
        vm.prank(USER);
        moodNft.mintNft();
        _;
    }

    function setUp() public {
        moodNft = new MoodNft(SAD_IMAGE_URI, HAPPY_IMAGE_URI);
    }

    // Check if the tokenURI return the correct json Metadata.
    function testViewTokenUri() public UserMintMoodNft {
        console.log(moodNft.tokenURI(0));
    }

    function testIfCheckRevertWhenNotOwnerCall() public UserMintMoodNft {
        vm.prank(BOB);
        vm.expectRevert();
        moodNft.flipMood();
    }

    function testFlipMoodByOwnerAndViewTokenUri() public UserMintMoodNft {
        vm.prank(USER);
        moodNft.flipMood();
        assertEq(uint(moodNft.getMoodNftState()), uint(MoodNft.Mood.SAD));
    }
}